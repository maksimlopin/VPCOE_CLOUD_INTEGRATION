class /VPCOE/CL_PROC_BASE_HU_OBSOLET definition
  public
...
public section.
  ...
  interfaces /VPCOE/IF_ENTITY_HU_PROC .
...
protected section.
  ...
  types:
    gty_t_handling_units TYPE SORTED TABLE OF gty_s_handling_unit
                                       WITH NON-UNIQUE KEY primary_key COMPONENTS delivery internal_number_hu
                                       WITH NON-UNIQUE SORTED KEY level COMPONENTS low_level .
  types:
    BEGIN OF gty_s_value_packcomp,
        value TYPE /vpcoe/if_entity_hu_proc=>gty_t_api_rp_packcomp_for_prod,
  ...
  types:
    gty_t_prd_assigments TYPE STANDARD TABLE OF /vpcoe/if_entity_hu_proc=>gty_s_api_sel_criteria WITH DEFAULT KEY .
  types:
  ...
  constants:
    BEGIN OF sc_item_type,
      ...
      END OF sc_item_type .
  data MS_PARAMETERS type /VPCOE/IF_PCKG_CMP_HU_LOAD=>GTY_S_HU_INPUT .
  ...
  data MT_CALCULATED_HU type /VPCOE/IF_ENTITY_HU_PROC=>GTY_T_HU_FOR_PRODUCT .
  constants MC_PACK_COMP_PRODUCT type STRING value '/api/replication/v2/GetPackagingCompositionsForProduct' ##NO_TEXT.

  methods RETRIEVE_COMPOSITION_DATA
    importing
      !IT_HANDLING_UNITS type /VPCOE/IF_ENTITY_HU_PROC=>GTY_T_HU_FOR_PRODUCT
    returning
      value(RT_HU_DATA) type /VPCOE/IF_ENTITY_HU_PROC=>GTY_T_API_RP_PACKCOMP_FOR_PROD .
...
CLASS /VPCOE/CL_PROC_BASE_HU_OBSOLET IMPLEMENTATION.


  METHOD /VPCOE/IF_ENTITY_HU_PROC~MAP_HU_DATA.
  ...
  METHOD /VPCOE/IF_UPH_ENTITY_PROC~DESERIALIZE_SELECTION_PARAMS.
    ...
    DATA ls_selection_params TYPE /vpcoe/if_pckg_cmp_hu_load=>gty_s_hu_input.
  ...
  METHOD /VPCOE/IF_UPH_ENTITY_PROC~PREPARE_PROCESS.
    DATA lv_ship_to_party    TYPE string.

    CALL FUNCTION 'CONVERSION_EXIT_PARVW_INPUT'
      EXPORTING
        input  = 'SH'
      IMPORTING
        output = lv_ship_to_party.  "partner function ship to party
    ...
    SELECT DISTINCT
             ...
             vbuk~wbstk     AS overall_goods_movement_status,
     ...
     FROM likp
             ...
             JOIN vbuk ON likp~vbeln = vbuk~vbeln
   WHERE likp~lfart    IN @ms_parameters-document_type
      ...
      AND likp~vbeln     IN @ms_parameters-sddoc
     ...
     INTO TABLE @DATA(lt_delivery_hdr).
    ...
    IF lt_delivery_hdr IS NOT INITIAL.
      ...
      SELECT lips~vbeln AS vbeln
        ...
        WHERE lips~vbeln   = @lt_delivery_hdr-id
          ...
          AND t001w~land1 IN @ms_parameters-plant_country
  ...
  METHOD /VPCOE/IF_UPH_ENTITY_PROC~PROCESS_PACKAGE.
    DATA lv_top            TYPE i.
    DATA lt_handling_units TYPE /vpcoe/if_entity_hu_proc=>gty_t_hu_for_product.
    ...
    DATA(lt_packcomp_data) = retrieve_composition_data( lt_handling_units ).

    rt_entity_data = /vpcoe/if_entity_hu_proc~map_hu_data( it_packcomp_data  = lt_packcomp_data
  ...
  METHOD CALCULATE_HU_DISTRIBUTION.
    ...
    mt_calculated_hu = VALUE /vpcoe/if_entity_hu_proc=>gty_t_hu_for_product( ).
...
*
*        IF lr_handling_unit->item_category <> /vpcoe/cl_proc_base_hu=>sc_item_type-addit_item
*           AND lr_handling_unit->item_category <> /vpcoe/cl_proc_base_hu=>sc_item_type-pack_mat.
*
*          DATA(ls_packed_product) = VALUE gty_s_product( product = lr_handling_unit->product
...
*
*          DATA(ls_aux_product) = VALUE gty_s_product( product = lr_handling_unit->product
...
*
*      "look deeper
*      resolve_hu_item( EXPORTING iv_hu    = lv_lower_level
*                                 iv_subord_quantity = lv_subord_quantity
*                       CHANGING ct_hu_items = ct_hu_items
...
*    ENDLOOP.
  ...
  METHOD RETRIEVE_COMPOSITION_DATA.
    ...
    ls_json = VALUE #( source                = ms_parameters-source_id
                       selection_period-from = ms_parameters-act_goods_mvt_date[ 1 ]-low
                       selection_period-to   = ms_parameters-act_goods_mvt_date[ 1 ]-high
    ...
    lo_http_util->get_http_client(
      EXPORTING
        ...
        iv_uri_suffix  =  mc_pack_comp_product
    ...
    lo_http_util->post_data_to_api(
      ...
      IMPORTING
        ...
        ev_response_txt = DATA(ls_response_txt) ).
