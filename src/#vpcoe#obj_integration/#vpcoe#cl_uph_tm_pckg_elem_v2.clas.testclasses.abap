...
CLASS /vpcoe/tcl_uph_tm_pckg_elem_v2 DEFINITION FOR TESTING ##CLASS_FINAL
  DURATION SHORT
  RISK LEVEL HARMLESS
.
  ...
  PRIVATE SECTION.
    DATA:
      f_cut TYPE REF TO /vpcoe/cl_uph_tm_pckg_elem_v2.  "class under test
...
ENDCLASS.       "tcl_Surdp_Uph_Tm_Pckg_Elem_V2


CLASS /VPCOE/tcl_uph_tm_pckg_elem_v2 IMPLEMENTATION.
  ...
  METHOD setup.
    f_cut =  NEW /vpcoe/cl_uph_tm_pckg_elem_v2( ) .
  ENDMETHOD.
...
| \} | .
  ...
  METHOD prepare_payload_test.
    ...
    ls_packelem_frac_data-isreinforced = abap_true.
    ls_packelem_frac_data-isexpanded = abap_true.

    "Packaging Functions
    ls_packelem_frac_data-functions = VALUE #( ( function = '10.100'
                                                 weightpercent = 45 )
                                               ( function = '10.107'
                                                 weightpercent = 25 ) ).

    DATA(lo_packelem_frac) = NEW /vpcoe/cl_ent_pckg_fraction( is_data = ls_packelem_frac_data iv_deleted = '' ).
