class /VPCOE/CL_PCKF_COMPANY_CODE definition
  public
  final
  create public .
...
CLASS /VPCOE/CL_PCKF_COMPANY_CODE IMPLEMENTATION.
  ...
  METHOD /VPCOE/IF_PCKF_EXEC_RETRIEVAL~EXECUTE_RETRIEVAL.
    ...
    IF iv_test = abap_false.
      MESSAGE s030(/vpcoe/pckf) WITH sy-datum sy-uzeit INTO DATA(lv_message).
    ELSE.
      MESSAGE s031(/vpcoe/pckf) WITH sy-datum sy-uzeit INTO lv_message.
    ENDIF.
    ...
    MESSAGE s024(/vpcoe/pckf) WITH lv_result_txt DISPLAY LIKE lv_result.
  ...
  METHOD RETRIEVE_ENTITY.
    ...
    IF iv_test = abap_false.
      ...
      DATA(ls_parameters) = VALUE /vpcoe/s_pckf_comp_code_input( ).
