class /VPCOE/CL_PCKF_RDP_CUST_EXEMP definition
  public
...
CLASS /VPCOE/CL_PCKF_RDP_CUST_EXEMP IMPLEMENTATION.


  METHOD /VPCOE/IF_PCKF_ENTITY_PROC~DESERIALIZE_SELECTION_PARAMS.
    CLEAR es_selection_params.

    DATA ls_selection_params TYPE /vpcoe/s_pckf_cstm_exemp_input.
  ...
  METHOD /vpcoe/if_pckf_entity_proc~prepare_retrieve.      "#EC CI_NOES
    ...
    DATA : lv_params_string         TYPE string,
           ls_input_params_frm_prot TYPE /vpcoe/s_pckf_cstm_exemp_input,
    ...
    IF mv_upload_mode EQ /vpcoe/if_pckf_entity_proc=>gc_upload_mode-gc_upload_mode_delta .
      ...
      ELSE.
        ...
        DATA lr_parameters TYPE REF TO /vpcoe/s_pckf_cstm_exemp_input.
  ...
  METHOD prepare_url_suffix.
    ...
    /vpcoe/if_pckf_entity_proc~get_parameter_value( EXPORTING iv_name = /vpcoe/if_pckf_entity_proc=>gc_param_exemption-cust_exemp_url  IMPORTING ev_value = ev_uri_suffix ).
    /vpcoe/if_pckf_entity_proc~get_parameter_value( EXPORTING iv_name = /vpcoe/if_pckf_entity_proc=>gc_param_exemption-customer        IMPORTING ev_value = lv_customer ).
    /vpcoe/if_pckf_entity_proc~get_parameter_value( EXPORTING iv_name = /vpcoe/if_pckf_entity_proc=>gc_param_exemption-report_category IMPORTING ev_value = lv_report_cat ).
    ...
    IF lv_customer IS NOT INITIAL.
      ...
      ENDIF.
      ev_query_string = |{ ev_query_string }customer eq '{ lv_customer }'|.
    ENDIF.
...
*    ev_query_string = `?$filter=reportCategory eq '` && lv_report_cat && `'` && `&?$filter=customer eq '` && lv_customer && `'` && `&$skip=` && lv_pack_size && `&$top=` && iv_pack_size.
