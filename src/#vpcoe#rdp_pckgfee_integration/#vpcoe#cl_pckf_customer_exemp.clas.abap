class /VPCOE/CL_PCKF_CUSTOMER_EXEMP definition
  public
  final
  create public .
...
CLASS /VPCOE/CL_PCKF_CUSTOMER_EXEMP IMPLEMENTATION.
  ...
  METHOD /VPCOE/IF_PCKF_EXEC_RETRIEVAL~EXECUTE_RETRIEVAL.
    ...
    IF iv_test = abap_false.
      MESSAGE s028(/vpcoe/pckf) WITH sy-datum sy-uzeit INTO DATA(lv_message).
    ELSE.
      MESSAGE s029(/vpcoe/pckf) WITH sy-datum sy-uzeit INTO lv_message.
    ENDIF.
    ...
    retrieve_entity(
      EXPORTING
        iv_entity_type = /vpcoe/if_pckf_entity_data=>gc_entities-gc_ent_cstm_exemp
        iv_upload_mode = iv_upload_mode
    ...
    MESSAGE s019(/vpcoe/pckf) WITH lv_result_txt DISPLAY LIKE lv_result.
  ...
  METHOD retrieve_entity.
    ...
    IF iv_test = abap_false.
      ...
      DATA(ls_parameters) = VALUE /vpcoe/s_pckf_cstm_exemp_input( ).
