CLASS /vpcoe/cl_pckf_cache DEFINITION
  PUBLIC
  CREATE PUBLIC .
...
CLASS /VPCOE/CL_PCKF_CACHE IMPLEMENTATION.
  ...
  METHOD /vpcoe/if_pckf_cache~clear.
    ...
    IF iv_ref_prot_uuid IS SUPPLIED AND iv_ref_prot_uuid IS NOT INITIAL.
      DELETE FROM /vpcoe/pckf_ench WHERE entity_type = @iv_entity_type AND ref_prot_uuid = @iv_ref_prot_uuid.
    ELSE.
      DELETE FROM /vpcoe/pckf_ench WHERE entity_type = @iv_entity_type.
    ENDIF.
  ...
  METHOD /vpcoe/if_pckf_cache~delete_entity.
    ...
    DELETE FROM /vpcoe/pckf_ench WHERE uuid = @iv_uuid.
  ...
  METHOD /vpcoe/if_pckf_cache~get_entities.
    ...
    SELECT uuid, entity_type, reportconfigid, jsondata, failed, creationdatetime FROM /vpcoe/pckf_ench
    ...
    LOOP AT lt_cache_entries REFERENCE INTO DATA(lr_cache_entry).
      ...
      CASE lr_cache_entry->entity_type.
        WHEN /vpcoe/if_pckf_entity_data=>gc_entities-gc_ent_pckg_fee.
          ...
          /vpcoe/cl_pckf_ent_pckg_fee=>/vpcoe/if_pckf_entity_data~json_to_entity(
            EXPORTING
              iv_json = lv_json_str
            IMPORTING
  ...
  METHOD /vpcoe/if_pckf_cache~get_references.
    ...
    SELECT entity_type, ref_prot_uuid, COUNT( * ) AS rec_cnt FROM /vpcoe/pckf_ench
      WHERE entity_type = @iv_entity_type
    ...
    LOOP AT lt_references REFERENCE INTO DATA(lr_reference).
      ...
      SELECT SINGLE MIN( creationdatetime ) FROM /vpcoe/pckf_ench WHERE entity_type = @lr_reference->entity_type AND ref_prot_uuid = @lr_reference->ref_prot_uuid INTO @lr_reference->creationdatetime.
      SELECT COUNT( * ) FROM /vpcoe/pckf_ench WHERE entity_type = @lr_reference->entity_type AND ref_prot_uuid = @lr_reference->ref_prot_uuid AND failed = @abap_true INTO @lr_reference->failed_cnt.
  ...
  METHOD /vpcoe/if_pckf_cache~set_entities.
    ...
    DATA: lt_update_tab TYPE STANDARD TABLE OF /vpcoe/pckf_ench,
          lt_insert_tab TYPE STANDARD TABLE OF /vpcoe/pckf_ench,
          ls_row_exists TYPE /vpcoe/pckf_ench,
    ...
    LOOP AT it_entities INTO DATA(lr_entity).
      ...
      SELECT SINGLE FOR UPDATE uuid, failed, creationdatetime FROM /vpcoe/pckf_ench WHERE uuid = @lv_act_uuid INTO CORRESPONDING FIELDS OF @ls_row_exists.
    ...
    IF lt_update_tab IS NOT INITIAL.
      ...
      UPDATE /vpcoe/pckf_ench FROM TABLE lt_update_tab.

    ENDIF.

    IF lt_insert_tab IS NOT INITIAL.

      INSERT /vpcoe/pckf_ench FROM TABLE lt_insert_tab.
    ENDIF.
  ...
  METHOD /vpcoe/if_pckf_cache~set_failed.
    ...
    UPDATE /vpcoe/pckf_ench SET failed = @abap_true WHERE uuid = @iv_uuid.
