class /VPCOE/CL_PCKF_RDP_COMP_CODE definition
  public
...
CLASS /VPCOE/CL_PCKF_RDP_COMP_CODE IMPLEMENTATION.


  METHOD /vpcoe/if_pckf_entity_proc~deserialize_selection_params.
    CLEAR es_selection_params.

    DATA ls_selection_params TYPE /vpcoe/s_pckf_comp_code_input.
  ...
  METHOD /VPCOE/IF_PCKF_ENTITY_PROC~PREPARE_RETRIEVE.      "#EC CI_NOES
    ...
    DATA : lv_params_string         TYPE string,
           ls_input_params_frm_prot TYPE /vpcoe/s_pckf_comp_code_input,
    ...
    IF mv_upload_mode EQ /vpcoe/if_pckf_entity_proc=>gc_upload_mode-gc_upload_mode_delta .
      ...
      DATA(lo_protocol_access) = /vpcoe/cl_pckf_factory=>get_instance( )->get_protocol_access( iv_upload_entity = /vpcoe/if_pckf_entity_data=>gc_entities-gc_ent_comp_code ).
      ...
      ELSE.
        ...
        DATA lr_parameters TYPE REF TO /vpcoe/s_pckf_comp_code_input.
  ...
  METHOD /VPCOE/IF_PCKF_ENTITY_PROC~RETRIEVE_PACKAGE.
    ...
    /vpcoe/if_pckf_entity_proc~get_parameter_value( EXPORTING iv_name = /vpcoe/if_pckf_entity_proc=>gc_param_exemption-rfc_des IMPORTING ev_value = rfc_des ).
  ...
  METHOD prepare_url_suffix.
    DATA: lv_criteria   TYPE string,
          ...
          lv_report_cat TYPE string,
          lv_pack_size  TYPE i,
          lv_message    TYPE string.

    CLEAR: ev_query_string, ev_uri_suffix, ev_query_finalized.

    /vpcoe/if_pckf_entity_proc~get_parameter_value( EXPORTING iv_name = /vpcoe/if_pckf_entity_proc=>gc_param_comp_code-comp_code_url   IMPORTING ev_value = ev_uri_suffix ).
    /vpcoe/if_pckf_entity_proc~get_parameter_value( EXPORTING iv_name = /vpcoe/if_pckf_entity_proc=>gc_param_comp_code-company_code    IMPORTING ev_value = lv_comp_code ).
    /vpcoe/if_pckf_entity_proc~get_parameter_value( EXPORTING iv_name = /vpcoe/if_pckf_entity_proc=>gc_param_comp_code-report_category IMPORTING ev_value = lv_report_cat ).
    ...
    IF lv_report_cat IS NOT INITIAL.
      ev_query_string = |?$filter=reportCategory eq '{ lv_report_cat }'|.
    ENDIF.

    IF lv_comp_code IS NOT INITIAL.
      IF ev_query_string IS NOT INITIAL.
        ev_query_string = |{ ev_query_string } and |.
      ELSE.
        ev_query_string = |?$filter=|.
      ENDIF.
      ev_query_string = |{ ev_query_string }companyCode eq '{ lv_comp_code }'|.
    ENDIF.
